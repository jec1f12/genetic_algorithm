###most of the grunt work of the ga is done here stuff that doesn't fit in crossover
...
def chunks(l, n):#little function to split lists
...
def mol_name_maker(pop,ng):
    ...
    for p in pop:
        ...
        temp_m.SetProp("_Name",str(i)+"_test_"+str(ng)) #setting some generic name
...
def coords_generator(mol_list):
    ...
    for m in mol_list:
        ...
        all_mol_coords.append((name,coords_list)) #add each mols coords to overall coords list
    return all_mol_coords


def xyz_generator(coords_list):
    '''generates a xyz file for a list of coordinates'''
    all_mol_files = []
    for name,mol in coords_list:
        mol_file = []
        ...
        all_mol_files.append((name,mol_file))
    return all_mol_files
    print "xyz files written"
...
def com_generator_neutral_grndst(coords_list):#comfiles aren't generated by this but by the celery tasks on iridis
    ''''write com object for neutral ground state calc for a list of coordinates, finishes when all are written'''
    all_mol_files = []
    i=1
...
def com_generator_neg_mol_neut_charge(coords_list):#these are 
...
def check_previous_gens_fp(new_mol_list):
    ...
    results_ps = glob.glob("*_results.p")
    results_ps_sorted = sorted(results_ps, key=lambda x: int(x.split("_")[1]),reverse=True)
    print results_ps_sorted
    prev_mols_list = []
    for re in results_ps_sorted:
        ml_list = cPickle.load( open(re, "rb" ) )
        for ml in ml_list:
            prev_mols_list.append(ml)
    ...
    for mol in new_mol_list:
        ...
        for pmb in uniq_prev_mols:
            if nmb == pmb:
                ...
                ea = pm.GetProp("_Ea")
                gm_name = pm.GetProp("_Gm_name")
                pm_name = pm.GetProp("_Name")
                mol.SetProp("_Name", pm_name)
                mol.SetProp("_Fit", fit)
                mol.SetProp("_Energy", en)
                mol.SetProp("_Ea", ea)
                mol.SetProp("_Gm_name",gm_name)
                matches.append(mol)
    print "number of matches "+str(len(matches))
    new_matches = matches + new_mol_list
    new_matches_set = set(new_matches)
...
def results_dumper(p_name,ml_sorted_by_en):
    ...
    for data in ml_sorted_by_en:
        ...
        try:
            ...
            eleca = "None"
            ea = mol.SetProp("_Ea", eleca)
        try:
            fit = mol.GetProp("_Fit")
        except KeyError:
            fitness = "None"
            fit = mol.SetProp("_Fit", fitness)
        try:
            gm_name = mol.GetProp("_Gm_name")
        except KeyError:
            gm_namo = "None"
            gm_name = mol.SetProp("_Gm_name", gm_namo)
        pm = AllChem.PropertyMol(mol)
        pm.SetProp("_Name", name)
        pm.SetProp("_Fit", fit)
        pm.SetProp("_Energy", en)
        pm.SetProp("_Ea", ea)
        pm.SetProp("_Gm_name", gm_name)
        dump_list.append(pm)
...
def csp_result_matcher(gen_result,mol_name_list):
    '''easier to write a csp version of this function'''
    mol_en = []
    print gen_result
    for mol in mol_name_list:
        for gr in gen_result:
            if mol.GetProp("_Name") == gr[0]:        
                print mol.GetProp("_Name"),gr[0]
                energy = gr[1]
                mol.SetProp("_Energy",energy)
                mol.SetProp("_Gm_name", gr[2])
        mol_en.append((mol,mol.GetProp("_Energy")))
    for ml in mol_en:
        print ml[0].GetProp("_Name"), ml[1]
